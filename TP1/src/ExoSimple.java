...
public class ExoSimple {
			...
			for (int j = i + 1; j < t.length; j++) {
				for (int k = j + 1; k < t.length; k++) {
					if (t[i] + t[j] + t[k] == 0) {
						...
						// Attention le resultat n'est pas lisible via
						// system.out mais utilisable par l'appellant
					}
				}
			}
		}
	...
	// On veut trouver dans une Hasmap dont le mot qui a la valeur associée le
	// plus grand
	//
	...
	//
	// On veut compter les mots reccurents et afficher le mot qui revient le
	// plus souvent
	...
	//public static double squareRootApproximation(n) { double r = n / 2;
    //while (abs( r - (n/r) ) > t) {
    //    r = 0.5 * (r + (n/r));
    //  }
    //return r; }
	...
	public static void main(String[] s) {
		...
		//System.out.println("r = " + squareRootApproximation(9));
