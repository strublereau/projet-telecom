...
public class validateSSH {
	...
	public static ArrayList<String> accessMachines(
			ArrayList<String> listeMachines) {

		ArrayList<String> listMachines_OK = new ArrayList<String>();
		String[] args1 = { "/usr/bin/ssh", "strublereau@<host>",
				"echo $((2+3))" };
		...
		for (String machine : listeMachines) {
			...
			try {
				...
				if (p.exitValue() == 0) {
					...
					System.out.println(" Machine OK " + machine);
				} else {
					String ligne;
					while ((ligne = br.readLine()) != null) {
						// ligne contient une ligne de sortie d'erreur
						System.out.println(" Machine KO " + ligne);
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
				System.out.println("Machine pas connecté" + machine);
